...
func _ready():
	...
	$s/Panel/h/InfectedTaco/itaco.play("default")
	$s/Panel/h/InfectedTaco/itacogolden.play("default")
func _process(_delta):
	...
	if Global.itaco_bought:
		if Global.itaco_equipped:
			$s/Panel/h/InfectedTaco/itaco_buy.text = "EQUIPPED"
		else:
			$s/Panel/h/InfectedTaco/itaco_buy.text = "EQUIP"
...
func _on_itaco_buy_pressed() -> void:
	if Global.itaco_bought:
		Global.dtaco_equipped = false
		Global.sbtaco_equipped = false
		Global.ntaco_equipped = false
		...
		Global.ftaco_equipped = false
		Global.staco_equipped = false
		Global.itaco_equipped = true
		$s/Panel/h/StormlockTaco/itaco_buy.text = "EQUIPPED"
		game.update_taco_skin()
		game.save_data()
		return

	if game.tacos >= itaco_cost:
		$s/Panel/h/StormlockTaco/itaco_buy.text = "EQUIP"
		Global.itaco_bought = true
		game.recalc()
	else:
		game.show_cost_warning($notenoughmoneylabel)
